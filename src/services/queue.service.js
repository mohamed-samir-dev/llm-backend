...
const certificateQueue = new Queue('certificate', { connection });
const liveReminderQueue = new Queue('live-reminder', { connection });
...
new Worker('live-reminder', async (job) => {
  ...
  for (const student of session.registeredStudents) {
    ...
    await notificationQueue.add('live-push', {
      userId: student._id,
      title: `🔴 جلسة مباشرة تبدأ الآن: ${session.title}`,
      body: 'انضم الآن للجلسة المباشرة',
      type: 'live_session',
...
module.exports = { emailQueue, notificationQueue, certificateQueue, liveReminderQueue };
